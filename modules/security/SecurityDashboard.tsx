...
export const SecurityDashboard: React.FC<Props> = ({ user, currentView = 'scanner' }) => {
  ...
  useEffect(() => {
    ...
    const alertInterval = setInterval(() => {
        const alerts = MockService.getActiveAlerts(user.estateId);
  ...
  useEffect(() => {
    if (chatUser) {
        const interval = setInterval(() => {
            const msgs = MockService.getMessages(user.id, chatUser.id);
            setMessages(msgs);
        }, 2000);
  ...
  const handleOpenChat = (resident: User) => {
      setChatUser(resident);
      const msgs = MockService.getMessages(user.id, resident.id);
      setMessages(msgs);
  };
  ...
  const handleSendMessage = (e: React.FormEvent) => {
      ...
      MockService.sendMessage(user.id, chatUser.id, newMessage);
  ...
  const handleResolveAlert = (alertId: string) => {
      MockService.resolveAlert(alertId);
      ...
      return (
        <div className="space-y-6">
            ...
            <div className="relative">
                ...
                <input 
                    className="w-full pl-10 p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            ...
            {/* UI Updated to match Screenshot (Darker Cards) */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredResidents.map(res => (
                    <div key={res.id} className="bg-slate-900 rounded-xl border border-slate-700 p-6 flex items-center justify-between shadow-lg">
                        <div>
                            <h4 className="font-bold text-white text-lg">{res.name}</h4>
                            <p className="text-sm text-slate-400">Unit: {res.unitNumber}</p>
                        </div>
                        <div className="flex gap-2">
                             <button 
                                onClick={() => handleOpenChat(res)}
                                className="w-10 h-10 rounded-full bg-slate-800 text-slate-300 hover:bg-indigo-600 hover:text-white flex items-center justify-center transition-colors border border-slate-600"
                            >
                            ...
                            <button 
                                ...
                                className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors border ${callingId === res.id ? 'bg-green-500 text-white animate-pulse border-green-400' : 'bg-slate-800 text-slate-300 hover:bg-green-600 hover:text-white border-slate-600'}`}
                            >
                                {callingId === res.id ? <Mic size={18} /> : <Phone size={18} />}
                            </button>
                        </div>
                    </div>
                ))}
            ...
            {chatUser && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[500px]">
                        <div className="p-4 bg-slate-900 text-white flex justify-between items-center border-b border-slate-800">
                             <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold">{chatUser.name}</h4>
                                    <p className="text-xs text-slate-400">Unit {chatUser.unitNumber}</p>
                                </div>
                             </div>
                             <button onClick={() => setChatUser(null)} className="text-slate-400 hover:text-white"><X /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-950">
                            {messages.length === 0 && <p className="text-center text-xs text-slate-400 mt-4">No messages yet. Start conversation.</p>}
                            {messages.map(msg => {
                                const isMe = msg.fromId === user.id;
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-tl-none'}`}>
                                            {msg.content}
                                            <p className={`text-[10px] mt-1 text-right ${isMe ? 'text-indigo-200' : 'text-slate-400'}`}>{new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                                        </div>
                                    </div>
                                )
                            })}
                        ...
                        <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2">
                             <input 
                                className="flex-1 bg-slate-100 dark:bg-slate-800 border-0 rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 dark:text-white"
      ...
      {activeAlerts.length > 0 && (
          <div className="space-y-2">
            {activeAlerts.map(alert => (
                <div key={alert.id} className="bg-red-600 text-white p-4 rounded-xl shadow-lg flex items-center justify-between animate-pulse">
                    <div className="flex items-center gap-3">
                        <Siren size={32} className="animate-bounce" />
                        <div>
                            <h3 className="font-bold text-lg">SOS ALERT</h3>
                            <p className="text-sm">Unit {alert.unitNumber} requested help!</p>
                        </div>
                    </div>
                    <Button onClick={() => handleResolveAlert(alert.id)} className="bg-white text-red-600 hover:bg-red-50 border-none font-bold">
                        RESOLVE
                    </Button>
                </div>
            ))}
