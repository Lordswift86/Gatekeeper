...
import { ScanLine, LogIn, LogOut, AlertTriangle, Car, BookOpen, Wifi, WifiOff, RefreshCw, Truck, Check, Phone, Search, Mic, Loader } from 'lucide-react';
...
export const SecurityDashboard: React.FC<Props> = ({ user, currentView = 'scanner' }) => {
  ...
  const loadLogs = async () => { if (!isOfflineMode) { try { const data = await api.getSecurityLogs(); setLogs(data); } catch (e) { } } };
  const loadDeliveries = async () => { /* Will use getPendingDeliveries when endpoint exists */ setDeliveries([]); };
  const loadResidents = async () => { try { const data = await api.getPendingUsers(); setResidents(data); } catch (e) { } };

  const performSync = async () => {
    if (isOfflineMode) return;
    setIsSyncing(true);
    try {
      // Sync offline actions when we have an endpoint
      if (offlineQueue.length > 0) {
        // Future: api.syncOfflineActions(offlineQueue);
        setOfflineQueue([]);
      }
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    if (isOfflineMode) {
      ...
      try {
        ...
        setScanResult({ success: true, pass: result });
      } catch (e: any) {
        setScanResult({ success: false, message: e.response?.data?.message || 'Invalid code' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleAction = async (action: 'ENTRY' | 'EXIT') => {
    ...
    if (isOfflineMode) {
      ...
      try {
        action === 'ENTRY' ? await api.processEntry(scanResult.pass.id) : await api.processExit(scanResult.pass.id);
  ...
  // Delivery Handlers
  const handleVerifyDelivery = async (passId: string) => {
    try {
      // Future: api.confirmDelivery(passId);
      ...
      console.error('Failed to verify delivery:', e);
    }
  };
  ...
  const handleCall = (residentId: string) => {
    ...
    // Future: api.initiateCall(residentId);
    // Simulate call flow
    setTimeout(() => {
      setCallingId(null);
  ...
  const handleManualEntry = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.addManualLog({ guestName: manualName, destination: manualDest, notes: manualNote });
